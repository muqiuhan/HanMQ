...
class Consumer(serverURI: URI, name: String):
  ...
  catch case e: InterruptedException => log.error(e.getMessage())
  ...
  def register(queueNames: List[String], append: Boolean): Unit =
    ...
    client.send(JSON.toJSONString(new Message(0, null, JSON.toJSONString(queueNames), utils.Date.getLocalTime())))
  end register

  def register(queueName: String, append: Boolean): Unit = register(List(queueName), append)

  /// Custom message handling
  def onMessage(action: java.util.function.Consumer[String]) = client.setOnMessageAction(action)

  def getRegisterInfo(): List[String] = registerInfo.toList
end Consumer
