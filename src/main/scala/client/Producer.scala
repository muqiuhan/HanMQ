...
class Producer(serverURI: URI, name: String):
  ...
  def send(message: String, _routingKey: String): Unit =
    ...
    client.send(JSON.toJSONString(new Message(1, message, _routingKey, utils.Date.getLocalTime())))
  end send

  def send(message: String): Unit = routingKey match
    case None             => log.error("Please set a default routing key.")
    case Some(routingKey) => send(message, routingKey)

  def setDefaultRoutingKey(_routingKey: String): Unit = routingKey = Some(_routingKey)
end Producer
